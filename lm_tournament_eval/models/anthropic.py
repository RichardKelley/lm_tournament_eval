# Almost entirely from: https://github.com/EleutherAI/lm-evaluation-harness/blob/main/lm_eval/models/anthropic_llms.py
...
#from lm_eval.models.openai_completions import LocalCompletionsAPI TODO
from lm_tournament_eval.models.utils import retry_on_specific_exceptions
...
please install anthropic via `pip install 'lm-eval[anthropic]'` or `pip install -e '.[anthropic]'`",
        ...
        for request in tqdm(_requests, disable=disable_tqdm):
            try:
                ...
                logging.critical(f"Server unreachable: {e.__cause__}")
                break
            except anthropic.APIStatusError as e:  # type: ignore # noqa: F821
                logging.critical(f"API error {e.status_code}: {e.message}")
                break
